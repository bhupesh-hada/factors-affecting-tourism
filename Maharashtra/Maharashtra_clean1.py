# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 16:04:11 2018
...
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_clean)):
        if corpus_clean[j] in wr_br:
            ...
            final_list_clean1_Maharashtra.append(statement)
            break 
        
        
actual_statement_clean_Maharashtra=[]
for i in index_clean:
    actual_statement_clean_Maharashtra.append(df[0][i])        
...
abc=corpus+final_list_clean1_Maharashtra
...
df_final=pd.DataFrame({'Review':actual_statement_clean_Maharashtra,'Rating':y_pred},columns=['Review','Rating'])
df_final.to_csv('cleanlines_Maharashtra_6.csv')  
