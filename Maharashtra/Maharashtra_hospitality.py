# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 16:15:19 2018
...
final_list_hospitality_Maharashtra=[]
index_hospitality=[]
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_hospitality)):
        if corpus_hospitality[j] in wr_br:
            ...
            final_list_hospitality_Maharashtra.append(statement)
            break    
         
             
    
actual_statement_hospitality_Maharashtra=[]
for i in index_hospitality:
    actual_statement_hospitality_Maharashtra.append(df[0][i])        
   
    
    
    
abc=corpus+final_list_hospitality_Maharashtra
...
df_final=pd.DataFrame({'Review': actual_statement_hospitality_Maharashtra,'Rating':y_pred},columns=['Review','Rating'])
df_final.to_csv('hospitality_Maharashtra_3.csv')
