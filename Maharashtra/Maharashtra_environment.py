# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 17:02:30 2018
...
final_list_environment1_Maharashtra=[]
...
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_environment)):
        if corpus_environment[j] in wr_br:
            ...
            final_list_environment1_Maharashtra.append(statement)
            break    
         
actual_statement_environ1_Maharashtra=[]
for i in index_environ1:
    actual_statement_environ1_Maharashtra.append(df[0][i])               
    
  
    
    
    
abc=corpus+final_list_environment1_Maharashtra
...
df=pd.DataFrame({'Review':actual_statement_environ1_Maharashtra,'Rating':y_pred},columns=['Review','Rating'])
df.to_csv('environment_Maharashtra_3.csv')
