...
b=[]
import codecs
with codecs.open('actual_statement_hospitality1.txt', "r",encoding='utf-8', errors='ignore') as fdata:
    c=fdata.readlines()
...
final_list_hospitality_Rajasthan=[]
index_hospitality=[]
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_hospitality)):
        if corpus_hospitality[j] in wr_br:
            ...
            final_list_hospitality_Rajasthan.append(statement)
            break    
         
             
    
actual_statement_hospitality_Rajasthan=[]
for i in index_hospitality:
    actual_statement_hospitality_Rajasthan.append(df[0][i])        
   
    
    
    
abc=corpus+final_list_hospitality_Rajasthan
...
df_final=pd.DataFrame({'Review': actual_statement_hospitality_Rajasthan,'Rating':y_pred},columns=['Review','Rating'])

df_final.to_csv('hospitality_rajasthan_3.csv')
