...
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_environment)):
        if corpus_environment[j] in wr_br:
            ...
            final_list_environment1.append(statement)
            break    
         
actual_statement_environ1=[]
for i in index_environ1:
    actual_statement_environ1.append(df[0][i])               
    
  
    
    
    
abc=corpus+final_list_environment1
...
df=pd.DataFrame({'Review':actual_statement_environ1,'Rating':y_pred},columns=['Review','Rating'])

df.to_csv('environment_Tamil_Nadu_3.csv')
