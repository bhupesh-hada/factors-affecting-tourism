# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 16:50:16 2018
...
final_list_clean1_Uttar_Pradesh=[]
index_clean=[]
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_clean)):
        if corpus_clean[j] in wr_br:
            ...
            final_list_clean1_Uttar_Pradesh.append(statement)
            break 
        
        
actual_statement_clean_Uttar_Pradesh=[]
for i in index_clean:
    actual_statement_clean_Uttar_Pradesh.append(df[0][i])        
...
abc=corpus+final_list_clean1_Uttar_Pradesh
...
df_final=pd.DataFrame({'Review':actual_statement_clean_Uttar_Pradesh,'Rating':y_pred},columns=['Review','Rating'])
  
df_final.to_csv('clean_Uttar-Pradesh.csv')   
