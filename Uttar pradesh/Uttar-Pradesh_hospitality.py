# -*- coding: utf-8 -*-
"""
Created on Wed Mar  7 10:36:30 2018
...
final_list_hospitality_Uttar_Pradesh=[]
index_hospitality=[]
for i in range(0,len(s)):
    ...
    for j in range(len(corpus_hospitality)):
        if corpus_hospitality[j] in wr_br:
            ...
            final_list_hospitality_Uttar_Pradesh.append(statement)
            break    
         
             
    
actual_statement_hospitality_Uttar_Pradesh=[]
for i in index_hospitality:
    actual_statement_hospitality_Uttar_Pradesh.append(df[0][i])        
   
    
    
    
abc=corpus+final_list_hospitality_Uttar_Pradesh
...
df_final=pd.DataFrame({'Review': actual_statement_hospitality_Uttar_Pradesh,'Rating':y_pred},columns=['Review','Rating'])

df_final.to_csv('hospitality_Uttar-Pradesh_new.csv')
